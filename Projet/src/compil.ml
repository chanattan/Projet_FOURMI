...
let rec eval (expr : expression Span.located) (env : environment) (file : out_channel) : value * environment = match expr with
  ...
  | Compare(comp, sp), _ -> let bool_val,new_env = (process_compare comp env file) in (match bool_val with
  ...
  | If((cond, spc), (prog, _)), _ -> let bool_val, new_env = eval (cond,spc) env file in (match bool_val with
...
and process_condition (condi : cond) (env : environment) (file_out : out_channel) : string * environment = match condi with
  ...
  | Food -> "Food", env
  | Rock -> "Rock", env
  | Marker(expr)  -> let value, new_env = (eval expr env file_out) in (match value with
...
| Eq(expr_left, expr_right) ->  let v1,new_env = eval (expr_left) env file in
...
and process_command (cmd: command) (env: environment) (file: out_channel) : value * environment = match cmd with
    ...
    Unit, env (* Obliger de renvoyer Unit comme on ne peut pas déterminer statiquement. 
       On ne prends pas en compte le changement d'environnement comme on est pas sûr à la compil du changement *)
	...
	| Pickup((fun_name,_), (arg_list,_)) -> let value,new_env = process_apply fun_name arg_list env file in value,new_env (* Ecrire le Pickup *)
...
in let new_env = aux prog env in let _,_ = process_program (!main_prog) new_env file_out in () (*évaluation de la fonction main*)